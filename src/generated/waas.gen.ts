...
export const WebrpcSchemaHash = "893fd582d8526f1bf5cc261fdacf73435a51f54c"
...
export interface WalletClient {
    ...
    sendTransaction(req: SendTransactionRequest, headers?: object, signal?: AbortSignal): Promise<SendTransactionResponse>

    callContract(req: CallContractRequest, headers?: object, signal?: AbortSignal): Promise<CallContractResponse>
...
export interface WalletBindingData {
...
export class Wallet implements WalletClient {
    ...
    queryKey = {
        ...
        sendTransaction: (req: SendTransactionRequest) => ['Wallet', 'sendTransaction', req] as const,
        callContract: (req: CallContractRequest) => ['Wallet', 'callContract', req] as const,
    ...
    sendTransaction = (req: SendTransactionRequest, headers?: object, signal?: AbortSignal): Promise<SendTransactionResponse> => {
        return this.fetch(
            this.url('SendTransaction'),
            createHttpRequest(JsonEncode(req), headers, signal)).then((res) => {
            return buildResponse(res).then(_data => {
                return JsonDecode<SendTransactionResponse>(_data, 'SendTransactionResponse')
            })
        }, (error) => {
            throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error instanceof Error ? error.message : String(error)}` })
        })
    ...
    callContract = (req: CallContractRequest, headers?: object, signal?: AbortSignal): Promise<CallContractResponse> => {
        return this.fetch(
            this.url('CallContract'),
            createHttpRequest(JsonEncode(req), headers, signal)).then((res) => {
            return buildResponse(res).then(_data => {
                return JsonDecode<CallContractResponse>(_data, 'CallContractResponse')
            })
        }, (error) => {
            throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error instanceof Error ? error.message : String(error)}` })
        })
