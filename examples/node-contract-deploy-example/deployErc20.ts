...
const defaultDeployerAddress =
  '0xce0042B868300000d44A59004Da54A005ffdcf9f' as const satisfies Address;
const deployerAddress =
  optionalAddress('DEPLOYER_ADDRESS', process.env.DEPLOYER_ADDRESS) ?? defaultDeployerAddress;
...
async function main() {
  ...
  console.log('[deploy] contract:', contractAddress);
  console.log(
    '[deploy] contract explorer:',
    `${Networks.amoy.explorerUrl}/address/${contractAddress}`
  );
  if (tx.txnHash) {
