...
export async function getPolygonBalances(walletAddress: Address): Promise<BalanceState> {
  ...
  const polBalance = balances.nativeBalances.find(
    (balance) => balance.chainId === POLYGON_NETWORK.id
  );
  const usdcBalance = balances.balances.find(
    (balance) => balance.contractAddress?.toLowerCase() === POLYGON_USDC.toLowerCase()
  );
  ...
  const positions = balancesResult.items
    .flatMap((balances) => {
      ...
      const position: EarnPosition = {
        ...
        marketName:
          market?.metadata?.name ?? balance.shareToken?.name ?? `${balance.token.symbol} position`,
...
function assertPolygonTransactions(transactions: ParsedYieldTransaction[], label: string): void {
  ...
  const unsupportedTransaction = transactions.find(
    (transaction) => transaction.chainId !== POLYGON_NETWORK.id
  );
