...
type ProviderEvent = 'accountsChanged' | 'chainChanged' | 'connect' | 'disconnect' | 'message';
...
export class OMSWalletProvider {
  ...
  private async sendTransaction(params: unknown): Promise<Hex> {
    const [request] = paramsAsTuple(params, 'eth_sendTransaction') as [
      OMSWalletProviderTransactionRequest
    ];
    ...
    const transactionChainId =
      request.chainId === undefined ? this.getChainId() : normalizeChainId(request.chainId);
    ...
    if ((transactionOptions as { waitForStatus?: unknown } | undefined)?.waitForStatus === false) {
      throw invalidParams(
        'waitForStatus: false is not supported by the OMS Wallet wagmi connector because wagmi sendTransaction must return an EVM transaction hash.'
      );
    }
  ...
  private async switchEthereumChain(params: unknown): Promise<null> {
    const [request] = paramsAsTuple(params, 'wallet_switchEthereumChain') as [
...
function missingTransactionHashMessage(response: { txnId?: unknown }): string {
  const suffix =
    typeof response.txnId === 'string' ? ` OMS transaction id: ${response.txnId}.` : '';
